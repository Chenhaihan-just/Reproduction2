...
def _mdc_elbow_info(singular_values, min_k: int = 1, max_k: Optional[int] = None):
    """Return MDC elbow details for debugging and the selected component count."""
    ...
    if r <= 2:
        return {
            ...
            "x": torch.linspace(0, 1, r, device=s.device).cpu().tolist()
            if r > 0
            else [],
...
def _encode_effective_prompt_embeds(
    pipe,
    prompt: str,
    device,
    ...
    out = torch.stack(
        [output.hidden_states[layer] for layer in hidden_states_layers],
        dim=1,
    )
    out = out.to(dtype=pipe.text_encoder.dtype, device=device)
    ...
    return prompt_embeds[0, valid_mask, :]


@dataclass
class SNodePromptPack:
    original_prompt_embeds: torch.Tensor
    steered_prompt_embeds: torch.Tensor
...
def prepare_snode_prompt_embeds(
    ...
    c = _encode_effective_prompt_embeds(
        pipe=pipe,
        prompt=prompt,
        device=device,
    ...
    c_null = _encode_effective_prompt_embeds(
        pipe=pipe,
        prompt="",
        device=device,
        max_sequence_length=max_sequence_length,
    ...
    if fixed_k is None:
        mdc_info = _mdc_elbow_info_skip_first(S, min_k=min_k, max_k=max_k)
    ...
    original_prompt_embeds = c.to(
        device=original_device,
        dtype=original_dtype,
    ).unsqueeze(0).clone()
    steered_prompt_embeds = c_hat.unsqueeze(0).clone()

    # Only needed when guidance_scale > 0, but safe to prepare here.
    negative_prompt_embeds = c_null.to(
        device=original_device,
        dtype=original_dtype,
    ).unsqueeze(0).clone()
